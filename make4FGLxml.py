...
class SourceList:
	...
	def make_model(self,galactic_file="gll_iem_v07.fits",galactic_name='gll_iem_v07',
                ...
                if os.path.dirname(galactic_file)=='':
                        galactic_file=os.path.join('$(FERMI_DIR)','refdata','fermi','galdiffuse',galactic_file)
                if os.path.dirname(isotropic_file)=='':
                        isotropic_file=os.path.join('$(FERMI_DIR)','refdata','fermi','galdiffuse',isotropic_file)
        ...
        def build_model(self):
                ...
                for source_name in self.sources.keys():
                        ...
                        source_out.setAttribute('ROI_Center_Distance',"{self.sources[source_name]['roi_distance']:.2f}")
                            ...
                            and not self.sources[source_name]['variable']):
                                ...
                                for spectrum_par in self.sources[source_name]['spectrum'].keys():
                                        if spectrum_par not in ['model','spectrum_file']:
                                                self.sources[source_name]['spectrum'].update([(spectrum_par+'_free',False)])
                                ...
                                for spectrum_par in self.sources[source_name]['spectrum'].keys():
                                        if spectrum_par  not in ['model','spectrum_file']:
                                                if spectrum_par in ['Prefactor','Integral','norm','Normalization']:
                                                        self.sources[source_name]['spectrum'].update([(spectrum_par+'_free',True)])
                                                else:
                                                        self.sources[source_name]['spectrum'].update([(spectrum_par+'_free',False)])
                        ...
                        #statement to catch things we haven't thought of, remove after successful testing
                                ...
                                for spectrum_par in source.sources[source_name]['spectrum'].keys():
                                        if spectrum_par not in ['model','spectrum_file']:
                ...
                galactic_spatial=Spatial(spatial_model='MapCubeFunction',spatial_file=self.galactic_model_file)
                ...
                if self.force_point_sources:
                        ...
                        print(f'Added {self.num_point} point sources and {self.num_extended} sources.')
        ...
        def get_sources_xml(self):
                ...
                for source,RA,DEC,distance in zip(self.sources,right_ascensions,declinations,self.source_distances):
                        ...
                        self.sources[name].update([('variable',float(src.getAttribute('Variability_Index'))>=self.variability_threshold),
